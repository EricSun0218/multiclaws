...
const plugin = {
    ...
    register(api) {
        ...
        const pluginService = {
            ...
            start: async (ctx) => {
                ...
                service.on("permission_prompt", async (event) => {
                    ...
                    try {
                        await (0, channel_prompt_1.sendChannelText)({
                            runtime: api.runtime,
                            route,
                            text: event.text,
                        });
                        ...
                        logger.warn(`[multiclaws] failed to send approval prompt via channel ${route.channelId}: ${String(error)}`);
                    }
                });
                ...
                service.on("task_completed_notification", async (event) => {
                    ...
                    try {
                        ...
                        logger.warn(`[multiclaws] failed to send task completion notification via channel ${route.channelId}: ${String(error)}`);
                    }
                });
                await service.start();
            },
        ...
        api.on("message_received", async (event, ctx) => {
            ...
            if (handled.handled) {
                api.logger.info(`[multiclaws] resolved approval request ${handled.requestId ?? "unknown"} -> ${handled.decision ?? "unknown"}`);
            }
        });
