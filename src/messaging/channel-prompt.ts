...
import { invokeGatewayTool, type GatewayConfig } from "../utils/gateway-client";
...
export class ApprovalRouteStore {
  ...
  constructor(stateDir?: string) {
    this.filePath = stateDir ? path.join(stateDir, "route.json") : null;
    if (this.filePath) {
      this.latestRoute = this.loadFromDisk();
    }
  }

  private loadFromDisk(): ApprovalRoute | null {
    if (!this.filePath) return null;
    try {
      const raw = fs.readFileSync(this.filePath, "utf8");
      const parsed = JSON.parse(raw) as ApprovalRoute;
      if (
        parsed &&
  ...
  private saveToDisk(route: ApprovalRoute): void {
    if (!this.filePath) return;
    try {
      const dir = path.dirname(this.filePath);
      fs.mkdirSync(dir, { recursive: true });
      fs.writeFileSync(this.filePath, JSON.stringify(route, null, 2), "utf8");
    } catch {
      // best-effort
    }
  }
 ...
 */
export async function deliverApprovalPrompt(params: {
  runtime: OpenClawPluginApi["runtime"];
  ...
  try {
    const filePath = path.join(stateDir, "multiclaws", "pending-approvals.json");
    let existing: Array<{ requestId: string; text: string; savedAtMs: number }> = [];
    try {
